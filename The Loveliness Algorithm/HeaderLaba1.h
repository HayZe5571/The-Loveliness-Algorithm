...
extern int createBook(vector<class Library>& books, int count);
extern void loadLibrary(vector<class Library>& books, const string& filename);
extern void saveLibrary(const vector<class Library>& books, const string& filename);
...
public:
    Library() : name("Отцы и дети"), author("Тургенев"), year(2020) {}
    Library(const string& n, const string& a, int y) {
        setName(n);
        setAuthor(a);
        setYear(y);
    }
    ...
    void setAuthor(const string& a) { if (!a.empty() && isAlphaString(a)) author = a; }
    void setYear(int y)             { if (y > 0 && y <= 2025) year = y; }
    ...
    static int deleteBook(vector<Library>& books, int count) {
        printbooks(books);
    ...
    static void printbooks(const vector<Library>& books) {
        cout << "\nСписок всех книг в библиотеке:" << endl;
        cout << string(30, '-') << endl;

        for (int i = 0; i < books.size(); i++) books[i].output(i);
    }
    static void searchByName(const vector<Library>& books) {
        string name;
    ...
    static void searchByAuthor(const vector<Library>& books) {
        string author;
    ...
    static void searchByYear(const vector<Library>& books) {
